...
export default class View extends Component {
	...
	initPreviewer(){
		...
		Array.prototype.map.call(imgs,(v,i)=>{
			...
			v.onclick=this.previewImg.bind(this,i);//绑定点击预览事件
			// const original=v.getAttribute('data-original-src');//获取原图，如果有原图则预览原图而不是缩略图
			// const w=v.getAttribute('data-w');//获取原图，如果有原图则预览原图而不是缩略图
			// const h=v.getAttribute('data-h');//获取原图，如果有原图则预览原图而不是缩略图
			// console.log('ori',original);
			// if(original){
			// 	list[i] = {src: original, w:w, h:h};
			// }else {
			// 	v.onload = () => {
